			...
			$datosJson = '{
				"data": [';
					...
					{						
						...
						{
							$botones = " <div class='btn-group'><button class='btn btn-warning btnEditarModelo' idModelo = '".$modelos[$i]["id_modelo"]."' data-toggle='modal' data-target = '#modalEditarModelo'><i class='fa fa-pencil'></i></button><button class='btn btn-danger btnEliminarModelo' idModelo = '".$modelos[$i]["id_modelo"]."'><i class='fa fa-times'></i></button></div>"; 
						}
						else
						{
							$botones = " <div class='btn-group'><button class='btn btn-warning btnEditarModelo' idModelo = '".$modelos[$i]["id_modulo"]."' data-toggle='modal' data-target = '#modalEditarModelo'><i class='fa fa-pencil'></i></button></div>";	
						}

						
						$datosJson .= '[
							"'.($i+1).'",							
							"'.$modelos[$i]['descripcion'].'",
							"'.$botones.'"
						],';	
					}
